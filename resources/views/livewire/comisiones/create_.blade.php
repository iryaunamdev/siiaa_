<div class="p-6 bg-white ease-out duration-400">
    ...
    <h5 class="text-lg text-gray-500 border-b mb-8 title-5">Integrantes</h5>
    {{$comision}}
    @foreach ($integrantes as $integrante)
        <button wire:click.prevent="editIntegrante({{$integrante->id}})">{{$integrante->nombre}}</button><br>
        {{$integrante->puesto}}
    @endforeach
</div>
...
<div class="bg-gray-50 px-4 py-3 sm:px-6 col-span-2 sm:flex sm:flex-row-reverse">
    ...
    <span class="flex w-full rounded-md shadow-sm sm:w-auto ml-3">
        <button wire:click.prevent="('editIntegrante')" type="button"
...
@if($editIntegrante)
    @include('livewire.comisiones.integrante')
@endif

@if($editDocumento)
    @include('livewire.comisiones.documento')
@endif
